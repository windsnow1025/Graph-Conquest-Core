  ...
  constructor(config: GameConfig) {
    ...
    for (const [name, entry] of this.gameMap.nodes) {
      if (this.nodeOwnership.get(name) === null) {
        const army = new Army(createUnits(garrisonStats, entry.data.income), config.neutralGarrison.unitType, garrisonStats, name);
  ...
  static fromJSON(save: GameSave): GameSystem {
    ...
    for (const [node, ownerName] of save.nodeOwnership) {
      if (ownerName === null) {
        ...
        const owner = game.allPlayers.find(p => p.name === ownerName)!;
